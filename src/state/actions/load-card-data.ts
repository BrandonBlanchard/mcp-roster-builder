...
type DeploymentData = [
  deploymentsByLetter: Record<string, string>,
  deploymentsById: Record<string, CrisisCard>,
];
...
export const loadCardDataActionCreator = (
  data: LoadCardDataActionArgs,
): LoadCardDataAction => ({
  type: 'loadCardDataAction',
  data,
...
export const loadCardDataReducer = (
  state: ApplicationState,
  { data = null, dispatch }: LoadCardDataActionArgs,
): ApplicationState => {
  if (data === null) {
    new Promise(async (resolve, reject) => {
      try {
        const data = await fetch(
          'https://crisis-bot.elindie.workers.dev/get-cards-v2',
        );
  ...
  // Hash all the things
  const hashByIdReducer = (
    agg: [Record<string, McpData>, string[]],
    card: McpData,
  ): [Record<string, McpData>, string[]] => {
    agg[0][card.id] = card;
    agg[1].push(card.id);
    return agg;
  };

  const [characterHash, characterIds] = data.characters.reduce(
    hashByIdReducer,
    [{}, []],
  );
  ...
  const [teamTactics, teamTacticIds] = data.teamTactics.reduce(
    hashByIdReducer,
    [{}, []],
  );
  const [infinityGems, gemIds] = data.gems.reduce(hashByIdReducer, [{}, []]);
  const [affiliations, affiliationIds] = data.affiliations.reduce(
    hashByIdReducer,
    [{}, []],
  );
  const [deploymentsByLetter, deploymentsById] = data.deployments.reduce(
    (agg: DeploymentData, deployment) => {
